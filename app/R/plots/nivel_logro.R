plot_nivel_logro <- function(df, facet, palette_fill, plot_theme) {
  df_count <- df %>%
    ...
    count(.data$curso, .data$tipo, .data$nivel_logro, name = "n")
  ...
  if (identical(facet, "off")) {
    df_count <- df_count %>% mutate(curso_tipo = paste(.data$curso, .data$tipo, sep = " | "))
    return(
      ggplot(df_count, aes(x = .data$curso_tipo, y = .data$n, fill = .data$nivel_logro)) +
        geom_col(position = "fill") +
        scale_y_continuous(labels = scales::percent) +
        scale_fill_manual(values = colors) +
        labs(x = "Curso | Tipo", y = "Proporción", fill = "Nivel de logro") +
        plot_theme +
